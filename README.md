...
4. Время начала акции в 0:00 по Гринвичу? Нужно уточнить это на картинке, чтобы у пользователей из TR и восточных поясов EU не было вопросов, мол почему по их локальному времени акция в 22:00 ещё не началась.
5. Я бы узнал, какие есть типы товаров. Чтоб проверить, что акция работает на все. 
...
Учитывая что у нас в задаче только 2 переменные (регион и дата), то для этой акции можно было бы сделать обычную таблицу: 
